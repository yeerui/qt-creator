...
ClangCompletionContextAnalyzer::ClangCompletionContextAnalyzer(
    ...
    , m_completionAction(PassThroughToLibClang)
    , m_positionForClang(-1)
...
void ClangCompletionContextAnalyzer::analyze()
{
    ...
    int endOfOperator = skipPrecedingWhitespace(startOfName);
    unsigned tokenKind = T_EOF_SYMBOL;
    int endOfExpression = startOfOperator(endOfOperator, &tokenKind, /*want function call =*/ true);

    if (isTokenForPassThrough(tokenKind)) {
        setActionAndClangPosition(PassThroughToLibClang, endOfOperator);
        return;
    } else if (tokenKind == T_DOXY_COMMENT) {
        setActionAndClangPosition(CompleteDoxygenKeyword, -1);
        return;
    } else if (tokenKind == T_POUND) {
        // TODO: Check if libclang can complete preprocessor directives
        setActionAndClangPosition(CompletePreprocessorDirective, -1);
        return;
    } else if (isTokenForIncludePathCompletion(tokenKind)) {
        setActionAndClangPosition(CompleteIncludePath, -1);
        // TODO: Return also endOfExpression position?
        return;
    }
    ...
    if (tokenKind == T_COMMA) { // For function hints
        textCursor.setPosition(endOfExpression);
    ...
    if (tokenKind == T_LPAREN) {
        ...
        if (expression.endsWith(QLatin1String("SIGNAL"))) {
            ...
            ExpressionUnderCursor expressionUnderCursor(m_languageFeatures);
            index = expressionUnderCursor.startOfFunctionCall(textCursor);
            setActionAndClangPosition(PassThroughToLibClangAfterLeftParen, findStartOfName(index));
        }
